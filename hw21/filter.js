...
const displayProducts = (arrayOfProducts) => {
    ...
    const productStr = arrayOfProducts.map(elem => `<div class="product">
    <img src="${elem.img}" alt="" />
    <span class="name">${elem.name}</span>
    <span class="priceText">${elem.price} $</span>
  </div>`).join("")
...
const setCategories = () => {
    ...
    const filteredCategories = allCategories.filter(
        (elem, index) => allCategories.indexOf(elem) === index
    )

    categories.innerHTML = filteredCategories.map(category => `<span class="category">${category}</span>`).join("")
